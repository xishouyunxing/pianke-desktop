...
if (-not (Test-Path -LiteralPath (Join-Path $llvmBin "libclang.dll"))) {
    throw "libclang.dll was not found: $llvmBin"
}
...
cd /d "$escapedRepo"
powershell -ExecutionPolicy Bypass -File scripts\prepare_opencv_runtime.ps1
if errorlevel 1 exit /b %errorlevel%
call tauri build --features opencv-orb
if errorlevel 1 exit /b %errorlevel%
call npm run check:no-python-bundle
"@

$cmdFile = Join-Path $env:TEMP ("pianke-rust-release-" + [guid]::NewGuid().ToString("N") + ".cmd")
try {
